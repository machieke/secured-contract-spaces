use detta_verify::{
    application_da_fixture_roots_manifest, APPLICATION_DA_FIXTURE_ROOTS_PATH,
    APPLICATION_DA_FIXTURE_ROOTS_SHA256_PATH,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let fixture_path = repo_root.join(APPLICATION_DA_FIXTURE_ROOTS_PATH);
    let attestation_path = repo_root.join(APPLICATION_DA_FIXTURE_ROOTS_SHA256_PATH);
    let fixture = application_da_fixture_roots_manifest();

    write_pretty_json(&fixture_path, &fixture)?;
    write_sha256sum(&attestation_path, &fixture_path)?;
    Ok(())
}
