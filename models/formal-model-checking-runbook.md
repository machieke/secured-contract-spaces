...
- proof release attestation target paths match the expected release set.
- proof release attestation target paths stay models-relative without traversal.
...
- proof release attestation lines use one `sha256sum` separator token.
...
- proof manifest root attestation filename matches the proof manifest artifact.
...
- proof manifest root attestation root matches the proof manifest JSON bytes.
- proof manifest root attestation helper parsing matches direct sha256sum
  splitting.
