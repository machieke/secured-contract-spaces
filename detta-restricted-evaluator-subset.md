...
`models/detta-restricted-evaluator-proof-trace-root.sha256` separately attests
the SHA-256 root of the compact JSON serialization of `report.trace`.
...
`models/detta-restricted-evaluator-resource-exhaustion.json` has this schema:

- `schema`: schema identifier;
- `schema_version`: numeric schema version;
- `evaluator`: evaluator implementation identifier;
- `cases`: resource exhaustion cases with name, step budget, script, expected
  error, and committed report.

The v1 fixture covers step-budget exhaustion after an attempted guarded write.
